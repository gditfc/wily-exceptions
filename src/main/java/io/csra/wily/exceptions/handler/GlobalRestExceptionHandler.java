...
public class GlobalRestExceptionHandler extends DefaultHandlerExceptionResolver {
    ...
    public ResponseEntity<Object> handleRequestTimeOutException(final RuntimeException e, final WebRequest request, final HttpServletResponse response) {
        return handleException(e, HttpStatus.REQUEST_TIMEOUT, request, response);
    }

    @ExceptionHandler(value = {MethodArgumentNotValidException.class})
    public ResponseEntity<Object> processValidationError(final MethodArgumentNotValidException e, final WebRequest request, final HttpServletResponse response) {
        final BindingResult result = e.getBindingResult();
        final String message = result.getFieldErrors().get(0).getDefaultMessage();
